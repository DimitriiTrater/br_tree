...
public:
  ...
  void optUsage(std::ostream &os, std::size_t indent = 0,
    ...
    for (it = itBegin; it != itEnd; ++it) {
      ...
      for (std::size_t i = 0; i < (std::max)(usageText.size(), desc.size());
           ++i) {
...
public:
  explicit RunContext(Ptr<IConfig const> const &cfg,
      ...
      : m_runInfo(cfg->name()), m_context(getCurrentMutableContext()),
        m_activeTestCase(NULL), m_config(cfg), m_reporter(reporter),
